...
def score(preds,y):
    ...
    return r,r2
def permutation_p(folds,y,r_obs,r2_obs=None,nperm=NPERM,seed=SEED):
    rng=np.random.default_rng(seed)
    null=np.empty(nperm)
    null_r2=np.empty(nperm)
    ss_tot=float(((y-y.mean())**2).sum())
    for i in range(nperm):
        yp=rng.permutation(y)
        preds=loo_predictions(folds,yp)
        null[i]=np.corrcoef(preds,yp)[0,1]
        null_r2[i]=1.0-float(((yp-preds)**2).sum())/ss_tot
    p=float((np.sum(null>=r_obs)+1)/(nperm+1))
    p_r2=(float((np.sum(null_r2>=r2_obs)+1)/(nperm+1))
          if r2_obs is not None else float("nan"))
    return p,null,p_r2,null_r2
def run_space(name,X,y):
    folds=make_folds(X)
    preds=loo_predictions(folds,y)
    r,r2=score(preds,y)
    p,null,p_r2,null_r2=permutation_p(folds,y,r,r2_obs=r2)
    print(f"{name}: n={len(y)} dim={X.shape[1]} r={r:+.4f} R2={r2:+.4f} "
          f"perm_p_r={p:.4f} perm_p_R2={p_r2:.4f} "
          f"(null r mean={null.mean():+.3f} 95pct={np.percentile(null,95):+.3f})")
    if np.isfinite(p_r2) and (p<0.05)!=(p_r2<0.05):
        print(f"  warning: r/R2 disagree at alpha=0.05 (p_r={p:.4f}, p_R2={p_r2:.4f})")
    return {"space":name,"n":len(y),"dim":X.shape[1],"r":r,"R2":r2,
            "perm_p":p,"perm_p_r2":p_r2}
