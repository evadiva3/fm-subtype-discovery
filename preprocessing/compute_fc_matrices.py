...
class preprocessBOLD:
    ...
    def splitConditions(self):
        ...
        conditionTimeSeries = [[] for _ in range(0, 7)]; conditionFrames = [[] for _ in range(0, 7)];
    ...
    # this method needs to use Ledoit-Wolf and proper Fisher z shifting;
    def buildFCMatrices(self):
        conditionFrames = self.splitConditions();
        fcMatrices = [[] for _ in range(7)]; lwMeasure = ConnectivityMeasure(kind="correlation", cov_estimator=LedoitWolf());
    ...
    # this is a new method that converts the matrix objects into PyTorch Geometric graph topologies;
    def buildSparse(self, fcMatrix, topK):
        ...
        clust = np.array([
            fcMatrix[i, np.argsort(np.abs(fcMatrix[i]))[-topK:]].mean()
            for i in range(numNodes)
        ]);
        x = torch.tensor(
            np.stack([meanConn, varConn, degree, clust, spectral], axis=1),
            dtype=torch.float,
        );
    ...
    # method to iterate and arrange by interpolated edge density;
    def execute(self):
        try:
